...
protected:
    ...
    {        
        ...
        if(t != nullptr)
        {            
            printTree(t->left, out, prePrint + (numofChild < 1 ? "    " : "│   "), 0, t->right == nullptr);            //打印前导输出            
            out << prePrint                //判断当前节点是否是最后一个子节点，输出不同的枝杈                
            << (numofChild < 1 ? "┌───" : "└───")                
            << t->element //<<","<<t->height               
            << std::endl;            //判断当前节点是否是最后一个子节点，添加不同的前导输出            
            //判断是否有兄弟节点            
    ...
    //     t = (t->right != nullptr) ? t->right : nullptr;
    //     delete oldNode;
    //     return newNode;
    // }
    ...
    BinaryNode *clone(BinaryNode *t) const {
        ...
        return new BinaryNode{t->element, clone(t->left), clone(t->right), clone(t->height)};
    }
};
